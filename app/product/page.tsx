...
import { RootState } from "../../redux/store"; // Redux store'un tipini içe aktarın
...
// Extra ve food items için sabit değerler
const itemsExtra: ExtraItem[] = [
  {
    id: 1,
    name: "Extra 1",
    price: 1,
...
const Index: React.FC = () => {
  const [prices, setPrices] = useState<number[]>([10, 20, 30]);
  const [price, setPrice] = useState<number>(prices[0]);
  const [size, setSize] = useState<number>(0);
  const [extraItems, setExtraItems] = useState<ExtraItem[]>(itemsExtra);
  ...
  const handleChange = (e: React.ChangeEvent<HTMLInputElement>, item: ExtraItem) => {
    const checked = e.target.checked;

    if (checked) {
      changePrice(item.price);
      setExtras((prevExtras) => [...prevExtras, item]);
    } else {
      changePrice(-item.price);
      setExtras((prevExtras) => prevExtras.filter((extra) => extra.id !== item.id));
    }
  };
  ...
  return (
    <div className="flex items-center md:h-[calc(100vh_-_88px)] gap-5 py-20 flex-wrap ">
      <div className="relative md:flex-1 md:w-[80%] md:h-[80%] w-36 h-36 mx-auto">
        <Image src="/images/pizza.jpeg" alt="" layout="fill" objectFit="contain" />
      </div>
      <div className="md:flex-1 md:text-start text-center">
        ...
        <div>
          ...
          <div className="flex items-center gap-x-20 md:justify-start justify-center">
            ...
            >
              <Image src="/images/size.png" alt="" layout="fill" className="bg-white" />
